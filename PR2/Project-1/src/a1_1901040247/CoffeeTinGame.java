...
public class CoffeeTinGame {
    ...
    static {
        Arrays.fill(BeansBag, 0, 20, BLUE);
        Arrays.fill(BeansBag, 21, 40, GREEN);
        Arrays.fill(BeansBag, 41, 60, REMOVED);
    }
     ...
     *   update tin with two beans are taken
     *   </pre>
     */
    ...
    public static char takeOne(char[] tin) {
        ...
        for (char bean : tin) if (bean != REMOVED) count++;
    ...
    public static void updateTin(char[] tin, char[] twoBeans) {
        ...
        if (b1 == b2) {
            if (b1 != BLUE) getBean(BeansBag, BLUE);
            putIn(tin, BLUE);
        } else {
            putIn(tin, GREEN);
        }
